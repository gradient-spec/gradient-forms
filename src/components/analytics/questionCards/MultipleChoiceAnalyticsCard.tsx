...
import { Award, TrendingDown, Table, BarChart2, Compass, Layers } from 'lucide-react';
...
export const MultipleChoiceAnalyticsCard: React.FC<Props> = ({ data }) => {
  const [exploreView, setExploreView] = useState<'distribution' | 'table' | 'summary'>('distribution');
  ...
  return (
    <div className="space-y-4 pt-1">
      {/* Top Highlights Strip (Most / Least Selected) */}
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
        ...
        {/* Progressive Disclosure: Explore Toggle */}
        <button
          type="button"
      ...
      {/* Contextual Explore Views Selector */}
      {isExploreOpen && (
      ...
      {/* 1. Default View: Categorical Distribution Bar Chart & List */}
      {exploreView === 'distribution' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-1">
          ...
          <div className="lg:col-span-5 space-y-1.5">
            <div className="text-[10px] font-mono uppercase text-slate-400 mb-1">Option Breakdown ({data.answeredCount} responses)</div>
            {stats.map((opt, i) => (
      ...
      {/* 2. Explore: Tabular Breakdown Table */}
      {exploreView === 'table' && (
